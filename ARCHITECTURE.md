...
**Model choice**: `gemini-2.5-flash` is the default text model — strong instruction-following at low latency/cost, appropriate for a portfolio-scale pipeline calling an LLM multiple times per digest (clustering summaries, captions, compliance classification). `generate_image_concept` returns a structured *text* description of an image (not pixels) in v1; the method signature is intentionally image-model-shaped so wiring in `gemini-2.5-flash-image` later is additive, not a redesign.
