...
class YouTubeConnector:
    """Fetches channel + recent uploads within lookback_days.

    Structured data: YouTube Data API v3, else yt-dlp.
    Operator theater: Playwright screenshots a rendered HTML digest (offline-safe).
    """
            ...
            try:
                async with YouTubeDataClient(self._api_key) as client:
                    ...
                    videos = await client.fetch_recent_videos(
                        channel, lookback_days=self._lookback_days
                    )
                ...
                await self._emit(
                    "log",
                    {"message": f"YouTube API failed; trying yt-dlp — {exc}"},
                )
                account = None
        ...
        if account is None:
            ...
            try:
                ...
                account, posts = await fetch_channel_via_ytdlp(
                    url, lookback_days=self._lookback_days
                )
    ...
    async def _operator_frames(self) -> None:
        ...
        if not self._accounts and not self._posts:
            return
        html_doc = _render_operator_html(self._accounts, self._posts, self._lookback_days)
...
def _render_operator_html(
    accounts: list[RawAccount], posts: list[RawPost], lookback_days: int
) -> str:
    ...
    for p in posts:
        cards.append(
            ...
            f"<p>{p['likes']} likes · {p['comments']} comments · {p['shares']} views</p>"
...
<p>Lookback {lookback_days}d · since {cutoff.date().isoformat()}</p>
<ul>{headers}</ul>
